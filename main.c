...
void kafka_conf_must_set(rd_kafka_conf_t *conf, const char *property,
                         const char *envvar) {
  char err[512];
...
int main(int argc, char *argv[]) {
  ...
  kafka_conf_must_set(conf, "bootstrap.servers",
                      "NOSDK_KAFKA_BOOTSTRAP_SERVERS");
  ...
  while (1) {
    ...
    if (err) {
      fprintf(stderr, "failed to commit offset: %s\n", rd_kafka_err2str(err));
    }
